## Subdomain Smuggling of LotL Payload
...
This attack is relevant for use cases where very aggressive policies have been applied to limit what URLs you can visit. In such scenarios, there is almost always an exclusion to allow communication with at least a few domains:
 ...
 - Software providers like Microsoft, Google, or Amazon that may have over-scoped allow rules.
...
Here are a few things you can do to make yourself more resilient against this tactic:
 ...
 - Make any URL restrictions granular enough to include sub-domains that could open up attack surface, in particular when they are not needed for how the specific kiosk functions.
 - Consider allow-listed subdomains and how they can be used, across other dependencies (e.g., if an enterprise software app you use, has subdomains that allow for just anyone to open an account for mail, chat, or to open tickets).
 - Create custom IOAs in your EDR for LotL binaries or executables running from unexpected places, such as Downloads.
 - Consider implementing an Application Control solution such as WDAC (Windows Defender Application Control) or another 3rd party alternative.
